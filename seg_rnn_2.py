    ...
    def calc_loss(self, batch_data, batch_label):
        ...
        for i in range(1, N + 1):
            ...
            for j in range(max(0, i - DATA_MAX_SEG_LEN), i):
                ...
                if FEATURE:
                    feature_data = FloatTensor(batch_data[j, :, WORD_DIM:])
        ...
        for batch_idx in range(B):
            ...
            for tag, length in label:
                ...
                if FEATURE:
                    feature_val = Variable(batch_data[chars, batch_idx, WORD_DIM:]).repeat(1, B, 1)
...
if __name__ == "__main__":
    ...
    fileHandler = logging.FileHandler(os.path.join(args.log_dir, 'logging','info.log'))
    logFormat = logging.Formatter('%(asctime)s %(levelname)-8s %(message)s')
    fileHandler.setFormatter(logFormat)
    rootlogger.addHandler(fileHandler)
    t = strftime("%Y-%m-%d %H:%M:%S")
    logging.info('The now time is ' +  t)
    board_path = os.path.join(args.log_dir, 'train', t)
